package rhs_algo.complete_search.boj14888Re;
...
public class Main {
    ...
    public static int calculator(int value, int op,int next){
        //계산된 값을 담을 변수인 value의 초기값은 첫번째 값으로 해주기
        if(op==1){
            //덧셈
            value=value+next;//다음값요소 더하기
        }else if(op==2){
            //뺄셈
            value=value-next;
        }else if(op==3){
...
//            int value=calculator();
            ...
            for(int cand=1;cand<=4;cand++){
                ...
                if(op[cand]>=1){
                    ...
                    record[k]=cand;
                    //완전탐색
                    //지금값과 연산자, 연산자 다음에 이어질 값을 넣어서 계산해주도록 calculator를 변형시키기
                    int newVal=calculator(value,cand,arr[k+1]);
